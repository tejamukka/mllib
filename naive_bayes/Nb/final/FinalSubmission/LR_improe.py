...
def getAllWords(datapath,stopwordsPath=None):
    ...
    for file in dirham:
        ...
        hamtokensList = [singleToken.lower() for singleToken in hamtokens if (len(singleToken) >2 and singleToken not in stopWords)]
        store = {}
        error = 0
    ...
    for file in dirspam:
        ...
        spamtokensList = [singleToken.lower() for singleToken in spamtokens if (len(singleToken) >2 and singleToken not in stopWords)]
        store = {}
...
def test(datapath):
    ...
    for file in dirham:
        ...
        regex = re.compile('\\W*')
        hamtokens = regex.split(sectionread)
        hamtokensList = [singleToken.lower() for singleToken in hamtokens if len(singleToken) > 2]
        sum = 0
        store = {}
        for token in hamtokensList:
            if (store.has_key(token)):
    ...
    for file in dirspam:
        ...
        regex = re.compile('\\W*')
        spamtokens = regex.split(sectionread)
        spamtokensList = [singleToken.lower() for singleToken in spamtokens if len(singleToken) > 2]
        sum = 0
        store = {}
        for token in spamtokensList:
            if (store.has_key(token)):
